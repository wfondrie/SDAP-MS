...
were imputed as the minimum LFQ intensity of the run if all of the
intensities at greater bait concentrations were also `0`. All other
missing values were assumed to be MAR, and were not considered in
modeling.
...
Thus, given the mass spec signal (raw intensity) of a protein, the total
mass spec signal (the sum of raw intensities for a run) we can estimate
the relative contributation of a single protein to the total protein
mass analyzed. Because the experiment was performed at a total protein
conentration of 1 ug/uL, we can then calculate the individual protein
concentrations using their molecular weights.
...
exBar <- examples %>%
    ...
    facet_wrap(~Bait, ncol = 2, scales = "free_x") +
