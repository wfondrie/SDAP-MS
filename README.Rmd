...
write_delim(highConf, "protRes.txt")
```
...
## Plot some binding curves
```{r plotExamples}
curveFun <- function(df, conc) {
    response <- (df$Rmax * conc) / (df$Kd + conc)
...
exampleCurves <- examples %>%
    select(Protein, bait, estimate, Rmax) %>%
    pmap(curveFun, conc = 1:2187)

exDat <- examples %>%
    select(Protein, bait, estimate, Rmax, CV) %>%
    left_join(rename(lfqModInput, Protein = `Protein IDs`)) %>%
...
lrp1bPlots <- exDat %>%
    ...
    geom_point() +
    facet_wrap(~`Gene names`, ncol = 3, scales = "free_y")
lrp1bPlots
